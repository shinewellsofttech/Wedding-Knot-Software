...
{
    [Route("api/V1/MaterialMaster")]
    [ApiController]
    public class ctlMaterialMaster : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "MaterialMaster";
        private readonly string sListFor = "MaterialMaster";
        private readonly string sAddEdit_ProcedureName = "AddEdit_MaterialMaster";
        private readonly mCommon mModel = new mCommon();
        public ctlMaterialMaster(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mMaterialMaster dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mMaterialMaster dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mMaterialMaster dataReceived)
        {
            try
            {
                /** Argument List **/
