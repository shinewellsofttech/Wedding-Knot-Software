...
namespace Sahakaar_API.Controllers.V1.Masters
{
    ...
    [ApiController]
    public class ctlVoucher : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "VoucherH";
        private readonly string sListFor = "VoucherData";
        private readonly string sAddEdit_ProcedureName = "AddEdit_Voucher";
        private readonly mCommon mModel = new mCommon();
        public ctlVoucher(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mVoucher dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mVoucher dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mVoucher dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                //Information
                dbPara.Add("VoucherDate", dataReceived.VoucherDate, DbType.Date);
                dbPara.Add("VoucherNo", dataReceived.VoucherNo, DbType.String);
                dbPara.Add("F_VoucherTypeMaster", dataReceived.F_VoucherTypeMaster, DbType.Decimal);
                dbPara.Add("ReferenceDate", dataReceived.ReferenceDate, DbType.Date);
                dbPara.Add("ReferenceNo", dataReceived.ReferenceNo, DbType.String);
                dbPara.Add("Narration", dataReceived.Narration, DbType.String);
                dbPara.Add("TotalDr", dataReceived.TotalDr, DbType.Decimal);
                dbPara.Add("TotalCr", dataReceived.TotalCr, DbType.Decimal);
                dbPara.Add("CurBal", dataReceived.CurBal, DbType.Decimal);
