...
{
    [Route("api/V1/CategoryMaster")]
    [ApiController]
    public class ctlCategoryMaster : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "CategoryMaster";
        private readonly string sListFor = "Category";
        private readonly string sAddEdit_ProcedureName = "AddEdit_CategoryMaster";
        private readonly mCommon mModel = new mCommon();
        public ctlCategoryMaster(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mCategoryMaster dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mCategoryMaster dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mCategoryMaster dataReceived)
        {
            try
            {
                /** Argument List **/
