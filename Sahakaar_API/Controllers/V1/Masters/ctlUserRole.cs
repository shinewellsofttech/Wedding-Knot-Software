...
namespace Sahakaar_API.Controllers.V1.Masters
{
    ...
    [ApiController]
    public class ctlUserRole : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "UserRole";
        private readonly string sAddEdit_ProcedureName = "AddEdit_UserRole";
        private readonly mCommon mModel = new mCommon();
        public ctlUserRole(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mUserRole dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mUserRole dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mUserRole dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("Code", dataReceived.Code, DbType.String);
                dbPara.Add("Description", dataReceived.Description, DbType.String);
                dbPara.Add("IsActive", dataReceived.IsActive, DbType.Boolean);
