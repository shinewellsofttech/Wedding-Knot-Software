...
{
    [Route("api/V1/GetTrialBalance")]
    [ApiController]
    public class ctlGetTrialBalance : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "VoucherH";
        private readonly string sListFor = "";
        private readonly string sAddEdit_ProcedureName = "GetTrialBalance";
        private readonly mCommon mModel = new mCommon();
        public ctlGetTrialBalance(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mGetTrialBalance dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mGetTrialBalance dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }

        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mGetTrialBalance dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("IsSummrized", dataReceived.IsSummrized, DbType.Boolean);
                dbPara.Add("IsShowOnlyOB", dataReceived.IsShowOnlyOB, DbType.Boolean);
                dbPara.Add("IsBrief", dataReceived.IsBrief, DbType.Boolean);
