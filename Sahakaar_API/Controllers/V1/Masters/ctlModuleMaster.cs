...
{
    [Route("api/V1/ModuleMaster")]
    [ApiController]
    public class ctlModuleMaster : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "ModuleMaster";
        private readonly string sAddEdit_ProcedureName = "AddEdit_ModuleMaster";
        private readonly mCommon mModel = new mCommon();
        public ctlModuleMaster(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mModuleMaster dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mModuleMaster dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mModuleMaster dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("Path", dataReceived.Path, DbType.String);
