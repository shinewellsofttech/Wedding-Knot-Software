...
{
    [Route("api/V1/GetTradingAndPL_Report")]
    [ApiController]
    public class ctlGetTradingAndPL_Report : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "VoucherH";
        private readonly string sListFor = "";
        private readonly string sAddEdit_ProcedureName = "GetTradingAndPL_Report";
        private readonly mCommon mModel = new mCommon();
        public ctlGetTradingAndPL_Report(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mGetTradingAndPL_Report dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mGetTradingAndPL_Report dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }

        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mGetTradingAndPL_Report dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("IsDetailed", dataReceived.IsDetailed, DbType.Decimal);
