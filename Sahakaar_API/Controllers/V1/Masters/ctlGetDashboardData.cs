...
{
    [Route("api/V1/GetDashboardData")]
    [ApiController]
    public class cltGetDashboardData : ControllerBase
    {
        ...
        private readonly string sAddEdit_ProcedureName = "GetDashboardData";
        private readonly mCommon mModel = new mCommon();
        public cltGetDashboardData(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mGetDashboardData dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mGetDashboardData dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mGetDashboardData dataReceived)
        {
            try
            {
                /** Argument List **/
