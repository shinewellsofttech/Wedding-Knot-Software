...
{
    [Route("api/V1/GetStockDetail")]
    [ApiController]
    public class cltGetStockDetail : ControllerBase
    {
        ...
        private readonly string sAddEdit_ProcedureName = "GetStockDetail";
        private readonly mCommon mModel = new mCommon();
        public cltGetStockDetail(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mGetStockDetail dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mGetStockDetail dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mGetStockDetail dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("F_ItemMaster", dataReceived.F_ItemMaster, DbType.Decimal);
                dbPara.Add("F_GSTGroupMaster", dataReceived.F_GSTGroupMaster, DbType.Decimal);
                dbPara.Add("F_CategoryMaste", dataReceived.F_CategoryMaste, DbType.Decimal);
