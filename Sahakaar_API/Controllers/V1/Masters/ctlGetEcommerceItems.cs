...
{
    [Route("api/V1/GetEcommerceItems")]
    [ApiController]
    public class ctlGetEcommerceItems : ControllerBase
    {
        ...
        private readonly string sAddEdit_ProcedureName = "GetEcommerceItems";
        private readonly mCommon mModel = new mCommon();
        public ctlGetEcommerceItems(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mGetEcommerceItems dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mGetEcommerceItems dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mGetEcommerceItems dataReceived)
        {
            try
            {
                /** Argument List **/
