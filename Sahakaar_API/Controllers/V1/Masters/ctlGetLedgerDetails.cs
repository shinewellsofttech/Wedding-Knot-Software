...
{
    [Route("api/V1/GetLedgerDetails")]
    [ApiController]
    public class cltGetLedgerDetails : ControllerBase
    {
        ...
        private readonly string sAddEdit_ProcedureName = "GetLedgerDetails";
        private readonly mCommon mModel = new mCommon();
        public cltGetLedgerDetails(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mGetLedgerDetails dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mGetLedgerDetails dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mGetLedgerDetails dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("ReportType", dataReceived.ReportType, DbType.Decimal);
                dbPara.Add("ViewType", dataReceived.ViewType, DbType.Decimal);
