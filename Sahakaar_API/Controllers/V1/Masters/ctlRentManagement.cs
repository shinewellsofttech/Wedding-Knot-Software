...
{
    [Route("api/V1/RentManagement")]
    [ApiController]
    public class ctlRentManagement : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "RentManagementH";
        private readonly string sListFor = "RentManagementData";
        private readonly string sAddEdit_ProcedureName = "AddEdit_RentManagement";
        private readonly mCommon mModel = new mCommon();
        public ctlRentManagement(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mRentManagement dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mRentManagement dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mRentManagement dataReceived)
        {
            try
            {
                /** Argument List **/
