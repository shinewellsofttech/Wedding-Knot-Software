...
{
    [Route("api/V1/ItemOpeningBalanceH")]
    [ApiController]
    public class ctlItemOpeningBalanceH : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "ItemOpeningBalanceH";
        private readonly string sListFor = "";
        private readonly string sAddEdit_ProcedureName = "AddEdit_ItemOpeningBalanceH";
        private readonly mCommon mModel = new mCommon();
        public ctlItemOpeningBalanceH(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mItemOpeningBalanceH dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mItemOpeningBalanceH dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mItemOpeningBalanceH dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("ItemOpeningBalanceNo", dataReceived.ItemOpeningBalanceNo, DbType.String);
                dbPara.Add("ItemOpeningBalanceDate", dataReceived.ItemOpeningBalanceDate, DbType.DateTime);
                dbPara.Add("Remarks", dataReceived.Remarks, DbType.String);
                dbPara.Add("TotalBalance", dataReceived.TotalBalance, DbType.Decimal);
                dbPara.Add("StrItemOpeningBalanceL", dataReceived.StrItemOpeningBalanceL, DbType.String);
