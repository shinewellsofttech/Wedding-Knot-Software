...
{
    [Route("api/V1/UnitMaster")]
    [ApiController]
    public class ctlUnitMaster : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "UnitMaster";
        private readonly string sListFor = "Country";
        private readonly string sAddEdit_ProcedureName = "AddEdit_UnitMaster";
        private readonly mCommon mModel = new mCommon();
        public ctlUnitMaster(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mUnitMaster dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mUnitMaster dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mUnitMaster dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("UnitName", dataReceived.UnitName, DbType.String);
                dbPara.Add("UnitCode", dataReceived.UnitCode, DbType.String);
                dbPara.Add("DecimalAllowed", dataReceived.DecimalAllowed, DbType.Boolean);
