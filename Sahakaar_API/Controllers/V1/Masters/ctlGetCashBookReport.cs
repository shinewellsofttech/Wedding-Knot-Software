...
{
    [Route("api/V1/GetCashBookReport")]
    [ApiController]
    public class cltGetCashBookReport : ControllerBase
    {
        ...
        private readonly string sAddEdit_ProcedureName = "GetCashBookReport";
        private readonly mCommon mModel = new mCommon();
        public cltGetCashBookReport(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mGetCashBookReport dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mGetCashBookReport dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mGetCashBookReport dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("CashLedgerId", dataReceived.CashLedgerId, DbType.Decimal);
