...
{
    [Route("api/V1/VerifyPayment")]
    [ApiController]
    public class ctlVerifyPayment : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "VerifyPayment";
        private readonly string sListFor = "Country";
        private readonly string sAddEdit_ProcedureName = "Update_PaymentSuccess";
        private readonly mCommon mModel = new mCommon();
        public ctlVerifyPayment(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mVerifyPayment dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mVerifyPayment dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mVerifyPayment dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("RazorpayOrderId", dataReceived.razorpay_order_id, DbType.String);
                dbPara.Add("RazorpayPaymentId", dataReceived.razorpay_payment_id, DbType.String);
                dbPara.Add("razorpay_signature", dataReceived.razorpay_signature, DbType.String);
