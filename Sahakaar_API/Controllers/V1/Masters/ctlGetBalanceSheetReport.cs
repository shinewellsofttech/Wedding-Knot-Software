...
{
    [Route("api/V1/GetBalanceSheetReport")]
    [ApiController]
    public class ctlGetBalanceSheetReport : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "VoucherH";
        private readonly string sListFor = "";
        private readonly string sAddEdit_ProcedureName = "GetBalanceSheetReport";
        private readonly mCommon mModel = new mCommon();
        public ctlGetBalanceSheetReport(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mGetBalanceSheetReport dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mGetBalanceSheetReport dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }

        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mGetBalanceSheetReport dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("DateAsOn", dataReceived.DateAsOn, DbType.DateTime);
