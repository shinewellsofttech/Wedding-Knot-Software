...
{
    [Route("api/V1/ItemSchemeMaster")]
    [ApiController]
    public class ctlItemSchemeMaster : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "ItemSchemeMasterH";
        private readonly string sListFor = "ItemSchemeMasterData";
        private readonly string sAddEdit_ProcedureName = "AddEdit_ItemSchemeMaster";
        private readonly mCommon mModel = new mCommon();
        public ctlItemSchemeMaster(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mItemSchemeMaster dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mItemSchemeMaster dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mItemSchemeMaster dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("SchemeJson", dataReceived.SchemeJson, DbType.String);
