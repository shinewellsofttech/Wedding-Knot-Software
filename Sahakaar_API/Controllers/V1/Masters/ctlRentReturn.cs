...
{
    [Route("api/V1/RentReturn")]
    [ApiController]
    public class ctlRentReturn : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "RentReturnH";
        private readonly string sListFor = "RentReturnData";
        private readonly string sAddEdit_ProcedureName = "AddEdit_RentReturn";
        private readonly mCommon mModel = new mCommon();
        public ctlRentReturn(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mRentReturn dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mRentReturn dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mRentReturn dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                //Information
                dbPara.Add("F_RentManagementH", dataReceived.F_RentManagementH, DbType.Decimal);
                dbPara.Add("EntryNo", dataReceived.EntryNo, DbType.String);
                dbPara.Add("EntryDate", dataReceived.EntryDate, DbType.Date);
                dbPara.Add("TillDate", dataReceived.TillDate, DbType.Date);
                dbPara.Add("F_LedgerMaster", dataReceived.F_LedgerMaster, DbType.Decimal);
                dbPara.Add("F_LedgerMaster_Tax", dataReceived.F_LedgerMaster_Tax, DbType.Decimal);
