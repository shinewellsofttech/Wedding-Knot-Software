...
namespace Sahakaar_API.Controllers.V1.Masters
{
    ...
    [ApiController]
    public class ctlUpdatePassword : ControllerBase
    {
        ...
        private readonly string sAddEdit_ProcedureName = "UpdatePassword";
        private readonly string sImageFolder = "MemberImages";
        private readonly mCommon mModel = new mCommon();
        public ctlUpdatePassword(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mUpdatePassword dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mUpdatePassword dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mUpdatePassword dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("CurrentPassword", dataReceived.CurrentPassword, DbType.String);
