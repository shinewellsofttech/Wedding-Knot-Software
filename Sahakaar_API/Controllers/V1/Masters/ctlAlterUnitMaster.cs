...
{
    [Route("api/V1/AlterUnitMaster")]
    [ApiController]
    public class ctlAlterUnitMaster : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "AlterUnitMaster";
        private readonly string sListFor = "Country";
        private readonly string sAddEdit_ProcedureName = "AddEdit_AlterUnitMaster";
        private readonly mCommon mModel = new mCommon();
        public ctlAlterUnitMaster(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mAlterUnitMaster dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mAlterUnitMaster dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mAlterUnitMaster dataReceived)
        {
            try
            {
                /** Argument List **/
                ...
                dbPara.Add("F_UnitMaster", dataReceived.F_UnitMaster, DbType.Decimal);
                dbPara.Add("F_AlterUnit", dataReceived.F_AlterUnit, DbType.Decimal);
                dbPara.Add("ConversionValue", dataReceived.ConversionValue, DbType.Decimal);
