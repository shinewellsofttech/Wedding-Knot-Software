...
{
    [Route("api/V1/SalesReturn")]
    [ApiController]
    public class ctlSalesReturn : ControllerBase
    {
        private readonly svcCommon _svc;
        private readonly IWebHostEnvironment _environment;
        private readonly string sTableName = "SalesReturnH";
        private readonly string sListFor = "SalesReturnData";
        private readonly string sAddEdit_ProcedureName = "AddEdit_SalesReturn";
        private readonly mCommon mModel = new mCommon();
        public ctlSalesReturn(svcCommon svc, IWebHostEnvironment environment)
        {
            this._svc = svc;
        ...
        public async Task<IActionResult> Add(string UserId, string UserToken, [FromForm] mSalesReturn dataReceived)
        {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Update(string UserId, string UserToken, [FromForm] mSalesReturn dataReceived, decimal Id)
        {
            return await Add_Edit(UserId, UserToken, Id: Id, dataReceived: dataReceived);
        }
        private async Task<IActionResult> Add_Edit(string UserId, string UserToken, decimal Id, mSalesReturn dataReceived)
        {
            try
            {
                /** Argument List **/
